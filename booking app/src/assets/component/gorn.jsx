...
function Home() {
  ...
  const availableEndTimes = selectedTime
    ? timeOptions.filter(
        (option) => parseFloat(option.value) > parseFloat(selectedTime.value)
      )
    : [];
  ...
  const fetchRooms = async () => {
    try {
      ...
      const buildings = Array.from(
        new Set(response.data.map((building) => building.building))
      ).map((buildingName) => {
        const buildingObj = response.data.find(
      ...
      const floors = Array.from(
        new Set(response.data.map((floor) => floor.floor))
      ).map((floorsName) => {
        const floorsObj = response.data.find(
          (floors) => floors.floor === floorsName
        );
        return { value: floorsObj.id, label: floorsName };
      });

      const rooms = Array.from(
        new Set(response.data.map((room) => room.name))
      ).map((roomsName) => {
        const roomsObj = response.data.find(
          (rooms) => rooms.name === roomsName
        );
        return { value: roomsObj.id, label: roomsName };
      });

      const types = Array.from(
        new Set(response.data.map((type) => type.type_name))
      ).map((typesName) => {
        const typeObj = response.data.find(
          (type) => type.type_name === typesName
        );
        return { value: typeObj.room_type_id, label: typesName };
      });
  ...
  const fetchFilteredRooms = async (event) => {
    ...
    if (!selectedDate || !selectedTime || !selectedTime2) {
      ...
      parseFloat(selectedTime.value) >= parseFloat(selectedTime2.value)
    ) {
      setModalMessage("เวลาเริ่มต้นต้องน้อยกว่าเวลาสิ้นสุด");
      setShowModal(true);
      return;
  ...
  useEffect(() => {
    ...
    if (selectedBuilding) {
      filtered = filtered.filter(
        (room) => room.building === selectedBuilding.label
      );
    }
    ...
    if (selectedType && selectedType !== "all") {
      filtered = filtered.filter(
        (room) => room.type_name === selectedType.label
      );
    }
    ...
    if (selectedPeople) {
      ...
      filtered = filtered.filter(
        (room) => room.cap >= minPeople && room.cap <= maxPeople
      );
    }
      ...
      <div className="row mb-3" style={{ marginTop: "20px" }}>
        ...
        <div className="col-md-2 d-flex align-items-center">
          <button
            ...
            style={{ boxShadow: "0 0 0.2rem rgba(0, 0, 0, 0.5)" }}
          >
            รีเซ็ตค่า
          </button>
        </div>
