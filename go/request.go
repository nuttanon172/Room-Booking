...
func updatebook(c *fiber.Ctx) error {
	...
	_, err := db.Exec("UPDATE booking SET status_id = (SELECT id FROM booking_status WHERE name = 'Waiting') , approved_id = (SELECT id FROM employee WHERE email=:1) WHERE id = :2", userEmail, bookingid)
	if err != nil {
